...
| IT-004 | 图表文件生成 | TC-T-016 | CLI 正常运行 | AWGN 默认模式三张 PNG 文件均非空 | `constellation.png`, `ber_curve.png`, `sync_peak.png` 均存在且 >0 byte | 中 |
...
| BER 曲线 trial 数 | 每个 SNR 至少 20 个 seed | `tests/test_plotting_statistics.py` |
...
| AT-008 | `tests/test_traceability_p0.py::test_cli_accepts_all_expected_awgn_plots` | 验证 AWGN CLI 三张承诺图表均有效时可成功。 |
...
## I. 2026-07-03 新增审计回归测试

本节为当前实现的最新验收补充，优先级高于历史测试计划中关于“BER 曲线单 seed”
和“至少两张图即可”的旧表述。

### I.1 CLI 参数与路径错误

新增 `tests/test_cli_validation.py`，覆盖：

| 场景 | 期望 |
|---|---|
| `--snr nan`、`inf`、`-inf`、`-9999`、`9999` | 返回非零，`stderr` 包含 `--snr`，无 traceback，不生成输出文件 |
| `--snr -10`、`12` | 合法运行，保留正常负 SNR 压力测试 |
| 非法 `--mod`、`--channel`、`--equalizer` | 返回非零，错误信息包含对应参数 |
| 输入文件不存在 | 返回非零，错误说明输入文件不存在 |
| 输入路径为目录 | 返回非零，错误说明不是普通文件 |
| 输出父路径为普通文件 | 返回非零，不生成 `metrics.json`、PNG 或 manifest |
| 输出路径本身为目录 | 返回非零，错误说明输出路径不能是目录 |

### I.2 字节级文本恢复

新增 `tests/test_recovery_and_stability.py::test_awgn_text_recovery_matches_original_bytes`，
用逐字节比较覆盖空文本、纯英文、纯中文、中英混合、emoji、UTF-8 BOM、换行符、
制表符、空字符、较长文本和 padding 边界长度。该测试防止 Windows 换行转换造成
`received.txt` 与输入文件 SHA-256 不一致。

### I.3 反硬编码与随机性

`tests/test_recovery_and_stability.py::test_dynamic_inputs_are_not_hardcoded`
动态生成仓库中不存在的随机文本，并用相同 seed 分别传输两个不同输入。断言输出随
输入变化且均来自完整链路恢复，避免固定读取 `Test.txt` 或预设答案。

### I.4 AWGN 与 Rayleigh 指标字段

`tests/test_recovery_and_stability.py::test_sync_and_metrics_fields_across_supported_modes`
覆盖 AWGN、Rayleigh+ZF、Rayleigh+MMSE 和双分支 MRC。至少检查
`sync_start_index`、`true_prefix_symbols`、`sync_error_symbols`、`sync_success`、
`checksum_pass`、`fer`、`payload_ber`、`predecode_ber`、`text_match_rate` 和
`frame_parse_strategy`。

### I.5 12 dB 多 seed 稳定性

`tests/test_recovery_and_stability.py::test_12db_awgn_stability_across_text_classes`
对短英文、短中文、中文英文 emoji 混合文本和当前 `Test.txt` 各运行 100 个 seed。
测试记录同步成功数、CRC 通过数、完整恢复数、失败 seed、平均 `predecode_ber` 和
最大同步误差。当前目标是四类文本均达到 100/100 同步成功、100/100 CRC 通过和
100/100 完整恢复；若未来随机失败，断言信息会输出完整 JSON 统计，不允许隐藏失败
样本。

### I.6 BER 曲线统计口径

`tests/test_plotting_statistics.py::test_ber_curve_uses_predecode_ber_statistics`
验证 `ber_curve.png` 的数据来自每个 SNR 至少 20 个 trial seed 的
`predecode_ber`，并检查 `ber_curve_data.json` 包含均值、标准差、最小值、最大值、
FER、CRC 通过率和完整恢复率。该测试防止把端到端 `payload_ber` 与理论 QPSK BER
直接混用。
